    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-40">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar descrição..." className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <select value={filterTipo} onChange={e => setFilterTipo(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div className="relative">
          ...
          <select value={filterMes} onChange={e => setFilterMes(e.target.value)} className="pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      {sorted.length === 0 ? (
        <div className="bg-white rounded-xl p-12 text-center shadow-sm border border-gray-100">
          ...
          {items.length === 0 && <button onClick={openNew} className="mt-4 text-blue-600 text-sm hover:underline">Criar primeiro lançamento</button>}
        </div>
      ) : (
                    ...
                    <tr key={l.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => openEdit(l)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded">
        ...
        <Modal title={editing ? 'Editar Lançamento' : 'Novo Lançamento'} onClose={() => setModalOpen(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input required value={form.descricao} onChange={set('descricao')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input required type="date" value={form.data} onChange={set('data')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Categoria *</label>
                <select value={form.categoria} onChange={set('categoria')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
