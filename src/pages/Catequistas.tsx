    ...
    <div className="space-y-5">
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl p-12 text-center shadow-sm border border-gray-100">
          ...
          {items.length === 0 && <button onClick={openNew} className="mt-4 text-blue-600 text-sm hover:underline">Cadastrar primeiro catequista</button>}
        </div>
      ) : (
              ...
              <div key={c.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                ...
                {turmasNomes.length > 0 && (
                  <div className="mt-3 pt-3 border-t flex flex-wrap gap-1">
                    {turmasNomes.map(n => (
                      <span key={n} className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded">{n}</span>
                    ))}
                  </div>
                )}
              </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input value={form.telefone} onChange={set('telefone')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nível / Formação</label>
              <input value={form.nivel} onChange={set('nivel')} placeholder="Ex: Crisma 1, Coordenador..." className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            {turmas.length > 0 && (
              <div>
